function varargout = guiex2e(varargin)
global temp pres volt
% GUIEX2E M-file for guiex2e.fig
%      GUIEX2E, by itself, creates a new GUIEX2E or raises the existing
%      singleton*.
%
%      H = GUIEX2E returns the handle to a new GUIEX2E or the handle to
%      the existing singleton*.
%
%      GUIEX2E('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUIEX2E.M with the given input arguments.
%
%      GUIEX2E('Property','Value',...) creates a new GUIEX2E or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before guiex2e_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to guiex2e_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help guiex2e
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @guiex2e_OpeningFcn, ...
                   'gui_OutputFcn',  @guiex2e_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before guiex2e is made visible.
function guiex2e_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to guiex2e (see VARARGIN)

% Choose default command line output for guiex2e
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes guiex2e wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = guiex2e_OutputFcn(hObject, eventdata, handles) 
...
% we will open a file-pick dialog and check the returned file.
 ...
 plot_data = handles.temp;
 handles.plot_data = handles.temp ;
% new data. Switch off smoothing and set smooth factor to minimum. 
 smm = get (handles.smooth, 'Min' ) ;
 set(handles.smooth ,'Value', smm ) ; 
 set (handles.smoothfactor , 'visible',  'off' ) ;
 sm_min = get (handles.smoothfactor , 'Min' );
 set( handles.smoothfactor , 'Value' , sm_min ); 
 plot(plot_data ) ;
